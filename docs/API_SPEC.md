...
Список всех навыков в каталоге.
...
Создать новый навык в каталоге.

**Auth:** да (только администраторы или модераторы могут создавать навыки)
...
## 4. Навыки пользователей (User Skills)

### GET /api/user-skills

Получить все записи о навыках пользователей (таблица user_skills).
...
### GET /api/user-skills/recommended/{userId}

Получить **рекомендуемые навыки от других пользователей**, отсортированные по релевантности для указанного пользователя.

**Auth:** да

**Параметры пути:** `userId` — ID пользователя, для которого нужны рекомендации

**Приоритеты сортировки (от высшего к низшему):**
1. **Комплементарные (наивысший приоритет):**
   - Пользователи, которые **преподают** навык, который **указанный пользователь хочет изучить** (type=teach у них, type=learn у указанного)
   - Пользователи, которые **хотят изучить** навык, который **указанный пользователь преподает** (type=learn у них, type=teach у указанного)
2. **Одна категория:** Навыки из тех же категорий, что и навыки указанного пользователя
3. **Тот же тип:** Навыки того же типа (teach/learn), что и у указанного пользователя для этого навыка
4. **Остальные:** Навыки, не попадающие в категории выше

**Пример:** `GET /api/user-skills/recommended/1`
...
## 5. Push-уведомления (Push Tokens)
...
Список всех бейджей с информацией о проценте пользователей, имеющих каждый бейдж.
...
| criteria    | string | Условие получения                              |
| level       | int    | Уровень бейджа (1-5)                           |
| user_count  | int    | Количество пользователей с этим бейджем        |
| total_users | int    | Общее количество пользователей в системе       |
| percentage  | float  | Процент пользователей с этим бейджем (0-100)   |

---

### GET /api/badges/user/{userId}

Бейджи пользователя. Возвращает только **уникальные бейджи** (по названию) с **максимальным уровнем**. Например, если у пользователя есть «Старожил» уровней 1, 2 и 3 — вернётся только «Старожил» уровня 3.
...
| image_url  | string | URL изображения бейджа                   |
| level      | int    | Уровень бейджа (1-5)                     |
| awarded_at | string | Дата выдачи (ISO 8601)                   |

**Примечание:** Если у пользователя есть несколько уровней одного и того же бейджа, возвращается только бейдж с наивысшим уровнем.
