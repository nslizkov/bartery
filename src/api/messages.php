...
if (!$id && $method === 'GET') {
    try {
        ...
        logApiError('Error fetching messages list', [
            'error' => $e->getMessage(),
            'file' => $e->getFile(),
            'line' => $e->getLine(),
            'user_id' => $user['id'],
...
if (is_numeric($id) && !$sub && $method === 'GET') {
    try {
        ...
        logApiError('Error fetching conversation', [
            'error' => $e->getMessage(),
            'file' => $e->getFile(),
            'line' => $e->getLine(),
            'user_id' => $user['id'],
            'other_id' => $otherId ?? null,
        ]);
        jsonResponse(['error' => 'Failed to fetch conversation'], 500);
    }
}
...
if (!$id && $method === 'POST') {
    try {
        ...
        try {
            ...
            logApiError('Push notification failed', [
