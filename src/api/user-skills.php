...
// GET /api/user-skills/recommended/{userId} - recommended skills from other users sorted by relevance (auth required)
...
// GET /api/user-skills/recommended/{userId} - Get recommended skills from other users
if ($id === 'recommended' && is_numeric($sub)) {
    try {
        $targetUserId = (int) $sub;
        ...
        // Get all skills from other users (excluding target user themselves)
        $stmt = $db->prepare('
        ...
        logApiError('Error fetching recommended skills', [
            'error' => $e->getMessage(),
            'file' => $e->getFile(),
            'line' => $e->getLine(),
